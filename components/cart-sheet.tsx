...
import { Offcanvas, Button, Image as RBImage } from "react-bootstrap"
    ...
    <Offcanvas show={open} onHide={closeCart} placement="end">
      ...
      <Offcanvas.Body>
          ...
          {items.map((i) => (
            <div key={i.id} className="d-flex gap-3">
              <div className="flex-shrink-0">
                <Image src={i.image || "/placeholder.svg"} alt={i.name} width={80} height={80} className="rounded border" />
              </div>
              ...
              <div>
                <Button variant="link" size="sm" onClick={() => removeItem(i.id)}>Quitar</Button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="d-grid gap-2">
          <Button variant="success" disabled={items.length === 0}>Proceder al pago</Button>
          <Button variant="outline-secondary" onClick={clear} disabled={items.length === 0}>Vaciar carrito</Button>
        </div>
      </Offcanvas.Body>
    </Offcanvas>
  )
